...
def obtain_text_features_and_palette():
    ...
    elif 'matterport_3d_40' in args.data_root:
        labelset = list(MATTERPORT_LABELS_40)
        palette = get_palette(colormap='matterport_160')
        dataset_name = 'matterport_40'
    elif 'matterport_3d_80' in args.data_root:
        labelset = list(MATTERPORT_LABELS_80)
        palette = get_palette(colormap='matterport_160')
        dataset_name = 'matterport_80'
    elif 'matterport_3d_160' in args.data_root:
        labelset = list(MATTERPORT_LABELS_160)
        palette = get_palette(colormap='matterport_160')
        dataset_name = 'matterport_160'
    elif 'nuscenes' in args.data_root:
        labelset = list(NUSCENES_LABELS_16)
        palette = get_palette(colormap='nuscenes16')
        dataset_name = 'nuscenes'
    elif 'kitti360' in args.data_root:
        labelset = list(KITTI360_LABELS_19)
        palette = get_palette(colormap='kitti360')
        dataset_name = 'kitti360'
    elif 'semantickitti' in args.data_root:
        labelset = list(SEMANTICKITTI_LABELS_20)
        palette = get_palette(colormap='semantickitti')
...
def distill(train_loader, model, optimizer, epoch):
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        # mask = mask[:output_3d.size(0)] # added to fix the error
        output_3d = output_3d[mask]

        if hasattr(args, 'loss_type') and args.loss_type == 'cosine':
            loss = (1 - torch.nn.CosineSimilarity(eps=1e-6)
        ...
        #     loss = torch.nn.KLDivLoss()(torch.log(output_3d), feat_3d)
        else:
            raise NotImplementedError
    ...
    mask = mask.to(coords.device) #added to fix the error
