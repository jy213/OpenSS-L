...
from run.local_distill import get_model
from models.disnet import state_dict_remove_module
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, x):
        weights = F.softmax(self.linear(x), dim=0)
        return weights
...
def precompute_text_related_properties(labelset_name):
    ...
    elif 'kitti360' in labelset_name:
        labelset = list(KITTI360_LABELS_19)
        palette = get_palette(colormap='kitti360')
...
def main():
    ...
    print("Current GPU Index: ", device)
    print("Current GPU Name: ", torch.cuda.get_device_name(device))
    '''Main function.'''
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.feature_type == 'fusion':
        ...
        try:
            model.load_state_dict(state_dict_remove_module(checkpoint['state_dict']), strict=True)
        ...
        for rep_i in range(args.test_repeats):
            ...
            for i, (coords, feat, label, feat_3d, mask, inds_reverse) in enumerate(tqdm(val_data_loader)):
                ...
                if feature_type == 'distill':
                    ...
                    attention_layer = AttentionLayer(input_dim=feat_fuse.shape[-1]).cuda()

                    # Calculate the weights
                    weights = attention_layer(feat_fuse)

                    # Apply the weights to the features
                    feat_fuse = weights * feat_fuse
                ...
                if vis_input:
                    # input_color = [(0, 0, 0) for _ in range(len(pcl))] # for nuscenes, no color
                    # export_pointcloud(os.path.join(save_folder, '{}_input.ply'.format(i)), pcl, colors=input_color)
                ...
                if vis_pred:
                    if mapper is not None:
                        logits_pred = logits_pred.clamp(max=mapper.shape[0]-1) # added for nuscenes fusion
