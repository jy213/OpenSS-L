...
def process_one_scene(data_path, out_dir, args):
    ...
    for img_id, cam in enumerate(tqdm(cam_locs)): # for each camera location
        ...
        # maps the 2D features to their corresponding 3D points
        feat_2d_3d = feat_2d[:, mapping[:, 1], mapping[:, 2]].permute(1, 0)

        # accumulates the 2D features for each visible 3D point
    ...
    mask[point_ids] = True
    mask_entire[mask_entire==True] = mask # stores the mask for the entire point cloud with visibility information
