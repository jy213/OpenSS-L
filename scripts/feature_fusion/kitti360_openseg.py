...
def process_one_scene(data_path, out_dir, args):
    ...
    cam_locs = ['image_00_0', 'image_00_1', 'image_00_2', 'image_00_3']
    ...
    for img_id, cam in enumerate(tqdm(cam_locs)):
        ...
        img_dir = join(img_dir_base, cam+'.png')
    ...
    torch.save({"feat": feat_bank[mask].half().cpu(),
            ...
            join(out_dir, scene_id +'_0.pt'))

    print(join(out_dir, scene_id +'_0.pt') + ' is saved!')
...
def main(args):
    ...
    args.cut_num_pixel_boundary = 1 # do not use the features on the image boundary
    args.feat_dim = 768 # CLIP feature dimension
    split = args.split
    data_dir = args.data_dir
    args.img_dim = img_dim

    data_root = join(data_dir, 'kitti360_3d')
    data_root_2d = join(data_dir,'kitti360_2d')
