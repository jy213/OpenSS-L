...
def process_one_scene(data_path, out_dir, args):
    ...
    sum_weights = torch.zeros((n_points_cur, 1), device=device)
    ...
    for img_id, img_dir in enumerate(tqdm(img_dirs)):
        ...
        mapping[:, 1:4], visibility_score = point2img_mapper.compute_mapping(pose, locs_in, depth)
        # vis_sum = visibility_score.sum()
        # print('vibility', visibility_score)
        ...
        # visibility_score = torch.from_numpy(visibility_score).to(device)
        mask = mapping[:, 3]
        vis_id[:, img_id] = mask
        ...
        # print(f"Shape of feat_2d_3d: {feat_2d_3d[mask!=0].shape}")
        # print(f"Shape of visibility_score: {visibility_score.shape}")
        visibility_score = torch.from_numpy(visibility_score).unsqueeze(-1).to(device)
        
        sum_features[mask!=0] += feat_2d_3d[mask!=0] * visibility_score
        sum_weights[mask!=0] += visibility_score
       

    # counter[counter==0] = 1e-5
    sum_weights[sum_weights==0] = 1e-5
    # feat_bank = sum_features/counter
    feat_bank = sum_features/sum_weights
