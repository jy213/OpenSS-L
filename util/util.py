...
def save_checkpoint(state, is_best, sav_path, filename='model_last.pth.tar'):
    ...
    if is_best:
        ...
        print("A new best model has been saved")
...
def get_palette(num_cls=21, colormap='scannet'):
    ...
    elif colormap == 'semantickitti':
        semantickitti_palette = []
        for _, value in SEMANTICKITTI_COLORMAP.items():
            semantickitti_palette.append(np.array(value))
        palette = np.concatenate(semantickitti_palette)
    elif colormap == 'kitti360':
        kitti360_palette = []
        for _, value in KITTI360_COLORMAP.items():
            kitti360_palette.append(np.array(value))
        palette = np.concatenate(kitti360_palette)
    else:
        n = num_cls
        palette = [0]*(n*3)
        for j in range(0,n):
