...
def evaluate(pred_ids, gt_ids, stdout=False, dataset='scannet_3d'):
    ...
    elif 'kitti360_3d' in dataset:
        CLASS_LABELS = KITTI360_LABELS_19
    elif 'semantickitti_3d' in dataset:
        CLASS_LABELS = SEMANTICKITTI_LABELS_20
    else:
        ...
        for i in range(N_CLASSES):
            ...
            try:
                if 'matterport' in dataset:
                    print('{0:<14s}: {1:>5.3f}   {2:>5.3f}'.format(label_name, class_accs[label_name], class_accs[label_name]))

                else:
                    print('{0:<14s}: {1:>5.3f}   {2:>5.3f}   ({3:>6d}/{4:<6d})'.format(
                        label_name,
